...
Logik uebernommen aus dem sitemap-tracker (models/robots.py), mit einer
Abweichung: der httpx-Client wird injiziert statt selbst geoeffnet. Beide
Sitemap-Scanner haben bereits einen konfigurierten Client (Proxy, Timeout,
User-Agent) - ein zweiter wuerde die Proxy-Konfiguration duplizieren.

Ausgewertet wird nur der `User-agent: *`-Block; das Tool tritt nicht unter
...
class RobotsChecker:
    ...
    async def load(self, client: httpx.AsyncClient, base_url: str) -> None:
        ...
        Args:
            client:
            Bereits konfigurierter httpx-Client (Proxy/Timeout/Header).
            base_url:
